...
export function Reader() {
  ...
  const onPointerUp = (event: React.PointerEvent) => { const start = pointer.current; if (!start || start.multi || Math.hypot(event.clientX - start.x, event.clientY - start.y) > 12 || menu || event.currentTarget !== event.target && (event.target as Element).closest('.reader-menu')) return; const fraction = event.clientX / innerWidth; if (fraction < .3) go(-1); else if (fraction > .7) go(1); else setMenu(true); };

...
const readerVersion = 'v2026.09.04.2';

export function Reader() {
  const { id } = useParams(); const navigate = useNavigate(); const [searchParams] = useSearchParams(); const [song, setSong] = useState<Song>(); const [setlist, setSetlist] = useState<Setlist>(); const [pages, setPages] = useState(1); const [menu, setMenu] = useState(true); const [documentTheme, setDocumentTheme] = useState('white'); const [offsetStep, setOffsetStep] = useState(50); const [tracks, setTracks] = useState<{ index: number; name: string; program: number }[]>([]); const [zoomInput, setZoomInput] = useState('100'); const container = useRef<HTMLDivElement>(null); const header = useRef<HTMLElement>(null); const pointer = useRef<{ x: number; y: number; scrollTop: number; multi: boolean } | undefined>(undefined);
  ...
  useEffect(() => { if (song) setZoomInput(String(Math.round(song.zoomLevel * 100))); }, [song?.zoomLevel]);
  ...
  const setZoomPercent = (value: string) => { setZoomInput(value); if (!value.trim()) return; const parsed = Number(value); if (Number.isFinite(parsed)) update({ zoomLevel: Math.min(2, Math.max(0.5, parsed / 100)) }); };
  const commitZoomPercent = () => setZoomInput(String(Math.round(song.zoomLevel * 100)));
